...
\t@ninailblanton\t`;
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-tox-orange/30 bg-tox-orange/10 text-tox-orange text-xs font-mono font-medium mb-3 backdrop-blur-md">
            <FileSpreadsheet className="h-3.5 w-3.5 text-tox-orange" />
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-tox-orange/30 bg-tox-orange/10 text-tox-orange hover:bg-tox-orange/20 text-xs font-semibold shadow-inner transition-all cursor-pointer"
          >
            <Sparkles className="h-3.5 w-3.5 text-tox-orange" />
      ...
      {/* Upload Zone */}
      {!parsedResult && (
        <div className="glass-panel rounded-3xl border-2 border-dashed border-white/20 hover:border-tox-orange p-10 sm:p-14 text-center space-y-4 transition-colors shadow-2xl group">
          <div className="h-14 w-14 rounded-2xl bg-tox-orange/10 border border-tox-orange/20 flex items-center justify-center text-tox-orange mx-auto shadow-inner group-hover:scale-105 transition-transform">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-3">
            <label className="cursor-pointer inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-tox-orange hover:bg-tox-orange-hover text-black text-xs font-semibold shadow-tox-orange transition-all">
            ...
            <button
              onClick={handleLoadSampleTsv}
              className="text-xs text-tox-orange hover:text-tox-orange-hover hover:underline font-semibold cursor-pointer"
            >
              Load bundled sample TSV directly
            </button>
          </div>
        </div>
      )}
      ...
      {parsedResult && (
        <div className="glass-panel rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl animate-in fade-in-50">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-4">
            <div>
              ...
              <div className="text-xs text-zinc-400 mt-1 font-mono">
                Columns recognized: <span className="text-tox-orange font-semibold">{parsedResult.headers.join(', ')}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-semibold text-xs shadow-lg transition-all disabled:opacity-50 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div className="p-4 rounded-2xl border border-white/[0.08] bg-obsidian-900/80 shadow-inner">
              <div className="text-[10px] uppercase font-mono font-medium text-zinc-400">Total Rows</div>
            ...
            <div className="p-4 rounded-2xl border border-white/[0.08] bg-obsidian-900/80 shadow-inner">
              <div className="text-[10px] uppercase font-mono font-medium text-zinc-400">Recognized Handles</div>
              <div className="text-2xl font-bold font-mono text-tox-orange mt-1">
                {parsedResult.totalEntries}
              </div>
            </div>

            <div className="p-4 rounded-2xl border border-white/[0.08] bg-obsidian-900/80 shadow-inner">
              <div className="text-[10px] uppercase font-mono font-medium text-zinc-400">Unique Influencers</div>
            ...
            <div className="p-4 rounded-2xl border border-white/[0.08] bg-obsidian-900/80 shadow-inner">
              <div className="text-[10px] uppercase font-mono font-medium text-zinc-400">Account Repeats</div>
            ...
            <div className="p-4 rounded-2xl border border-white/[0.08] bg-obsidian-900/80 shadow-inner">
              <div className="text-[10px] uppercase font-mono font-medium text-zinc-400">Ambiguous Cells</div>
          ...
          <div className="p-4 rounded-2xl border border-tox-orange/20 bg-tox-orange/5 text-xs text-zinc-300 space-y-1">
            <span className="font-semibold text-tox-orange block font-mono">
              Cross-Account Normalization Verified:
            </span>
            <p className="text-zinc-400 leading-relaxed">
              Handles appearing in multiple columns (such as <span className="font-mono text-tox-orange font-bold">@notboredindc</span> and <span className="font-mono text-tox-orange font-bold">@azfoodie</span>) are consolidated into single canonical influencer records, while their respective outreach events are preserved under each account context. Dates will remain <span className="font-mono text-zinc-200 font-bold">NULL</span> per specification ("Historical / Date unavailable").
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-mono uppercase tracking-wider text-zinc-400">
                Staged Outreach Events Sample (First 15 of {parsedResult.totalEntries})
              </h3>
            ...
            <div className="max-h-80 overflow-y-auto rounded-2xl border border-white/[0.08] bg-obsidian-950/60">
              <table className="w-full text-left border-collapse dense-table">
                <thead>
                  <tr className="border-b border-white/[0.08] bg-obsidian-900/80 font-mono text-xs text-zinc-400">
                    <th className="py-3 px-4">#</th>
                    <th className="py-3 px-4">Handle</th>
                    <th className="py-3 px-4">Account Context</th>
                    <th className="py-3 px-4">Header Context</th>
                    <th className="py-3 px-4">Repeat Status</th>
                    <th className="py-3 px-4">Date Strategy</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.04] font-sans">
                  {parsedResult.outreaches.slice(0, 15).map((entry, idx) => (
                    <tr key={idx} className="hover:bg-white/[0.02] text-xs transition-colors">
        ...
        {batches.length === 0 ? (
          ...
          <div className="space-y-3">
            {batches.map((b) => (
              <div
                key={b.id}
                className="p-4 rounded-2xl border border-white/[0.06] bg-obsidian-900/60 hover:bg-white/[0.02] flex items-center justify-between text-xs transition-colors"
              >
                <div>
